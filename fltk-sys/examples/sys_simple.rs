...
unsafe extern "C" fn cb(_wid: *mut button::Fl_Widget, data: *mut c_void) {
    ...
    unsafe { frame::Fl_Box_set_label(frame, c"Hello World".as_ptr() as *const _) };
}
...
fn main() {
    unsafe {
        ...
        fl::Fl_lock();
        let win = window::Fl_Window_new(100, 100, 400, 300, c"Window".as_ptr() as *const _);
        let frame = frame::Fl_Box_new(0, 0, 400, 200, std::ptr::null());
        let but = button::Fl_Button_new(160, 220, 80, 40, c"Click".as_ptr() as *const _);
