...
impl Tree {
    ...
    pub fn extend_selection_dir(
        ...
        assert!(!self.was_deleted());
        if from._inner.is_null() || to._inner.is_null() {
            return Err(FltkError::Internal(FltkErrorKind::FailedOperation));
        }
        unsafe {
    ...
    pub fn set_user_icon<Img: ImageExt>(&mut self, image: Option<Img>) {
        ...
        if let Some(mut image) = image {
            ...
            unsafe {
                image.increment_arc();
                Fl_Tree_set_usericon(self._inner, image.as_image_ptr() as *mut _)
            }
        } else {
    ...
    pub fn set_open_icon<Img: ImageExt>(&mut self, image: Option<Img>) {
        ...
        if let Some(mut image) = image {
            ...
            unsafe {
                image.increment_arc();
                Fl_Tree_set_openicon(self._inner, image.as_image_ptr() as *mut _)
            }
        } else {
    ...
    pub fn set_close_icon<Img: ImageExt>(&mut self, image: Option<Img>) {
        ...
        if let Some(mut image) = image {
            ...
            unsafe {
                image.increment_arc();
                Fl_Tree_set_closeicon(self._inner, image.as_image_ptr() as *mut _)
            }
        } else {
...
impl TreeItem {
    ...
    pub unsafe fn from_raw(ptr: *mut Fl_Tree_Item) -> Option<TreeItem> {
        if ptr.is_null() {
            ...
            let tree_inner = Fl_Tree_Item_tree(ptr);
            let tracker =
                fltk_sys::fl::Fl_Widget_Tracker_new(tree_inner as *mut fltk_sys::fl::Fl_Widget);
            assert!(!tracker.is_null());
            let tree = Tree {
                _inner: tree_inner as *mut Fl_Tree,
                _tracker: tracker,
            };
    ...
    pub fn was_deleted(&self) -> bool {
        unsafe {
            ...
            let is_root = self._is_root;
            if self._tree.was_deleted() {
                return true;
            }
            if is_root {
