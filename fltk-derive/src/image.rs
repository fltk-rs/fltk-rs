    ...
    ($name: tt, $flname: tt) => {
            ...
            fn drop(&mut self) {
                if !self.was_deleted() {
                    ...
                    if *self.refcount.get_mut() < 1 {
                        unsafe {
                            concat_idents!($flname, _delete)(self.inner);
                        }
                    }
                }
            }
        ...
        unsafe impl ImageExt for $name {
            fn copy(&self) -> Self {
                assert!(!self.was_deleted());
                unsafe {
                    let img = concat_idents!($flname, _copy)(self.inner);
                    assert!(!img.is_null());
                    $name {
                        inner: img,
            ...
            fn draw(&mut self, arg2: i32, arg3: i32, arg4: i32, arg5: i32) {
                assert!(!self.was_deleted());
                unsafe { concat_idents!($flname, _draw)(self.inner, arg2, arg3, arg4, arg5) }
            }

            fn draw_ext(&mut self, arg2: i32, arg3: i32, arg4: i32, arg5: i32, cx: i32, cy: i32) {
                assert!(!self.was_deleted());
                unsafe {
                    concat_idents!($flname, _draw_ext)(self.inner, arg2, arg3, arg4, arg5, cx, cy)
                }
            }
            ...
            fn to_rgb_data(&self) -> Vec<u8> {
                assert!(!self.was_deleted());
                unsafe {
                    let ptr = concat_idents!($flname, _data)(self.inner);
                    assert!(!ptr.is_null());
                    assert!(!(*ptr).is_null());
            ...
            fn to_raw_data(&self) -> *const *const u8 {
                assert!(!self.was_deleted());
                unsafe { concat_idents!($flname, _data)(self.inner) as *const *const u8 }
            }
            ...
            fn count(&self) -> i32 {
                assert!(!self.was_deleted());
                unsafe { concat_idents!($flname, _count)(self.inner) }
            }

            fn data_w(&self) -> i32 {
                assert!(!self.was_deleted());
                unsafe { concat_idents!($flname, _data_w)(self.inner) }
            }

            fn data_h(&self) -> i32 {
                assert!(!self.was_deleted());
                unsafe { concat_idents!($flname, _data_h)(self.inner) }
            }

            fn depth(&self) -> ColorDepth {
                assert!(!self.was_deleted());
                unsafe { mem::transmute(concat_idents!($flname, _d)(self.inner) as u8) }
            }

            fn ld(&self) -> i32 {
                assert!(!self.was_deleted());
                unsafe { concat_idents!($flname, _ld)(self.inner) }
            }

            fn inactive(&mut self) {
                assert!(!self.was_deleted());
                unsafe { concat_idents!($flname, _inactive)(self.inner) }
            }

            unsafe fn delete(mut img: Self) {
                assert!(!img.inner.is_null());
                concat_idents!($flname, _delete)(img.inner);
